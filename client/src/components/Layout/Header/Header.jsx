...
const Header = () => {
  ...
  return (
    <section
      className={`z-50 bg-black/70 hover:bg-black backdrop-blur-lg fixed left-0 py-2.5 w-full transition-all duration-300 ${
        isScrolled ? 'top-0' : 'top-[-100%]'
      }`}
    >
      <header className="container flex items-center justify-between px-4 mx-auto">
        {/* Logo */}
        <h1 className="text-sm text-[#eeeeee] font-CrimsonPro">
        ...
        <nav className="items-center hidden gap-10 md:flex">
          {/* Menu Lists */}
          <ul className="flex items-center gap-10 text-xs text-[#eeeeee] uppercase">
            {menuLists.map((item) => (
              <li key={item.id}>
                <Link
                  to={item.to}
